...
static counter_t false_deps;        /* cumulative LSQ occupancy */
...
sim_reg_stats(struct stat_sdb_t *sdb)   /* stats database */
{
  ...
  stat_reg_counter(sdb, "LSQ_count", "cumulative LSQ occupancy",
    ...
    stat_reg_counter(sdb, "false_depss", "cumulative LSQ occupancy",
                     &false_deps, /* initial value */0, /* format */NULL);
...
static int* load_dep(int currentLoadIndex)
{
		  ...
		  && OPERANDS_READY(lsq))
		{
		  ...
		  readyq_enqueue(lsq);
               ...
               )
            {
                /* panic("store immediately ready"); */
                /* put operation on ready list, ruu_issue() issue it later */
                int * array = load_dep(old_lsq_tail);
