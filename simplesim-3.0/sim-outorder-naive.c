...
static int* load_issue(int currentLoadIndex)
{
   ...
   /* struct Map* currMap = storeSetMap;
    ...
    for(iter=0; iter<SSM_num; iter++){// iterate through the map
        if(currMap->key == LSQ[currentLoadIndex].PC){
            found = 1;
            //printf("FOUND");
                ...
                for (iter3=0; iter3<n_store_unresolved; iter3++){ // check to see if a particular store in the set is in flight
                    if (LSQ[store_unresolved_index[iter3]].PC == storeset[iter2]){
                        ...
                        if(STORE_ADDR_READY(&LSQ[store_unresolved_index[iter3]]) &&
                           LSQ[store_unresolved_index[iter3]].addr != LSQ[currentLoadIndex].addr){
                            break;
                        }
                        else{
                            conflict =1;
...
#include "machine.def"
        ...
        if (!spec_mode)
        {
            ...
            if (((MD_OP_FLAGS(op) & (F_MEM|F_LOAD)) == (F_MEM|F_LOAD)) && results[0] == 1 && results[1] ==1)
            {
                /* entering mis-speculation mode, indicate this and save PC */
